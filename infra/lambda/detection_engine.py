...
def resolve_source_names(query: str) -> str:
    ...
    return re.sub(r"(['\"])([\w_]+)\1", replace_source, query)
...
def run_detections(data_source):
    """Run all detection rules against the specified data source."""
    setup_duckdb()
    print(f"Running detections against source: {data_source}")

    rules = load_rules()
    enabled_rules = [r for r in rules if r.get("enabled", True)]
    print(f"Running {len(enabled_rules)} enabled rules against source: {data_source}")

    for rule in enabled_rules:
        try:
            # Resolve source names in the query
...
def lambda_handler(event, context):
    ...
    # Check if this is an EventBridge Event (Scheduled)
        ...
        # Run detections against all processed data (or partition by date/time if needed)
        all_processed_data = f"s3://{S3_BUCKET}/processed/*/*.parquet"
        run_detections(all_processed_data)
